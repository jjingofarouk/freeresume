...
const IconButton = ({ onClick, tooltipText, size = "medium", className = "", children }: IconButtonProps) => {
  ...
  return (
    <motion.button
      className={`relative group rounded-lg bg-[var(--theme-dark-blue)] shadow-sm hover:shadow-md transition-shadow duration-300 ${sizeClass} ${className}`}
      onClick={onClick}
    ...
    >
      {children}
      <motion.span
        className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[var(--theme-black)] text-[var(--theme-gold)] text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200"
...
export const ShowIconButton = ({
  ...
  return (
    <IconButton onClick={() => setShow(!show)} tooltipText={tooltipText}>
      <Icon className="h-5 w-5 text-[var(--theme-gray)]" aria-hidden="true" />
...
export const MoveIconButton = ({
  ...
  return (
    <IconButton
      ...
      size={size}
    >
      <Icon className={`${sizeClassName} text-[var(--theme-gray)]`} aria-hidden="true" />
      <span className="sr-only">{tooltipText}</span>
    </IconButton>
  );
};
...
export const DeleteIconButton = ({
  ...
  return (
    <IconButton onClick={onClick} tooltipText={tooltipText} size="small">
      <TrashIcon className="h-4 w-4 text-[var(--theme-gold)]" aria-hidden="true" />
      <span className="sr-only">{tooltipText}</span>
    </IconButton>
  );
};
...
export const BulletListIconButton = ({
  ...
  return (
    <IconButton
      ...
      tooltipText={tooltipText}
      size="small"
      className={showBulletPoints ? "!bg-[var(--theme-secondary-blue)]" : ""}
    >
      <ListBulletIcon
        className={`h-4 w-4 ${showBulletPoints ? "text-[var(--theme-gold)]" : "text-[var(--theme-gray)]"}`}
        aria-hidden="true"
      />
      <span className="sr-only">{tooltipText}</span>
    </IconButton>
